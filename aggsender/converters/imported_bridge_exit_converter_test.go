...
func TestConvertClaimToImportedBridgeExit(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			exit, err := ConvertToImportedBridgeExitWithoutClaimData(tt.claim)
			if tt.expectedError {
				require.Error(t, err)
			} else {
...
func TestGetImportedBridgeExits(t *testing.T) {
			...
			claims: []bridgesync.Claim{
				{
					...
					GlobalIndex:         bridgesync.GenerateGlobalIndex(2, 1),
		...
		},
		{
			...
			claims: []bridgesync.Claim{
				...
				},
				{
					...
					Metadata:            []byte("data"),
					GlobalIndex:         bridgesync.GenerateGlobalIndex(0, 2),
					GlobalExitRoot:      common.HexToHash("0x7891"),
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			exits, err := ConvertToImportedBridgeExits(t.Context(), tt.claims, common.HexToHash("0x7891"), mockL1InfoTreeQuery)
