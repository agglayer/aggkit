	...
	testCases := []struct {
		...
		},
		{
			...
			mockFn: func(mockStorage *mocks.AggSenderStorage,
				...
				mockL2BridgeQuerier.EXPECT().GetBridgesAndClaims(ctx, uint64(1), uint64(10), types.CertificateTypeFEP).Return([]bridgesync.Bridge{{}}, []bridgesync.Claim{
					{
		...
		},
		{
			...
			mockFn: func(mockStorage *mocks.AggSenderStorage,
				...
				mockStorage.EXPECT().GetLastSentCertificateHeader().Return(nil, nil).Once()
				mockL2BridgeQuerier.On("GetLastProcessedBlock", ctx).Return(uint64(10), nil)
				mockL2BridgeQuerier.EXPECT().GetBridgesAndClaims(ctx, uint64(1), uint64(10), types.CertificateTypeFEP).Return([]bridgesync.Bridge{}, []bridgesync.Claim{}, nil)
				mockL1InfoDataQuery.EXPECT().GetFinalizedL1InfoTreeData(ctx).Return(
					treetypes.Proof{},
					&l1infotreesync.L1InfoTreeLeaf{
						BlockNumber: l1Header.Number.Uint64(),
		...
		},
		{
			...
			mockFn: func(mockStorage *mocks.AggSenderStorage,
				...
				mockL2BridgeQuerier.On("GetLastProcessedBlock", ctx).Return(uint64(10), nil)
				mockL2BridgeQuerier.EXPECT().GetBridgesAndClaims(ctx, uint64(6), uint64(10), types.CertificateTypeFEP).Return([]bridgesync.Bridge{{}}, []bridgesync.Claim{{
					GlobalIndex:     big.NewInt(1),
					GlobalExitRoot:  ger,
					MainnetExitRoot: mer,
					RollupExitRoot:  rer,
		...
		},
		{
			...
			mockFn: func(mockStorage *mocks.AggSenderStorage,
				...
				mockStorage.EXPECT().GetLastSentCertificateHeader().Return(&types.CertificateHeader{ToBlock: 5}, nil).Once()
				mockL2BridgeQuerier.On("GetLastProcessedBlock", ctx).Return(uint64(10), nil)
				mockL2BridgeQuerier.EXPECT().GetBridgesAndClaims(ctx, uint64(6), uint64(10), types.CertificateTypeFEP).Return(
