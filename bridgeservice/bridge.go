...
// GetSetClaimsHandler returns a paginated list of set claims (L2 only).
//
// @Summary Get set claims
// @Description Returns set claims for the L2 network, paginated
// @Tags set-claims
// @Param page_number query int false "Page number"
// @Param page_size query int false "Page size"
// @Param global_index query string false "Filter by global index"
...
func (b *BridgeService) GetBridgeByDepositCountHandler(c *gin.Context) {
	...
	case b.networkID:
		...
		if err != nil {
			...
			b.logger.Errorf("failed to get bridge by deposit count %d for L2 network (ID=%d): %v",
				depositCount, networkID, err)
			statusCode = http.StatusInternalServerError
			c.JSON(statusCode, gin.H{"error": fmt.Sprintf("failed to get bridge by deposit count: %s", err)})
			return
		}
