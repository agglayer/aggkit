...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/config": {
            "get": {
                "description": "Returns a sanitized view of the bridge service configuration: component sync\nparameters and contract addresses, deduplicated by network. Never exposes RPC\nURLs, DB paths, private keys or any other internal/sensitive configuration value.",
                "produces": [
                    "application/json"
                ],
                "tags": [
        ...
        "/settlements": {
            "get": {
                "description": "Returns a paginated list of the L1 transactions that settled a new local exit\nroot for this network, most recent first. tx_hash and block_timestamp are omitted\nfor settlements synced before this endpoint was added, and are never backfilled\nfor those.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/types.SettlementsResult"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
    ...
    "definitions": {
        ...
        "types.L1ContractsConfig": {
            ...
            "properties": {
                "BridgeAddr": {
                    "description": "BridgeAddr is the address of the bridge smart contract on L1",
                    "type": "string",
                    "example": "0xabcdef1234567890abcdef1234567890abcdef12"
                },
                "GlobalExitRootAddr": {
                    "description": "GlobalExitRootAddr is the address of the GlobalExitRoot manager contract on L1",
                    "type": "string",
                    "example": "0xabcdef1234567890abcdef1234567890abcdef12"
                },
                "RollupManagerAddr": {
                    "description": "RollupManagerAddr is the address of the RollupManager/AgglayerManager contract",
                    "type": "string",
                    "example": "0xabcdef1234567890abcdef1234567890abcdef12"
                }
            }
        ...
        "types.L2ContractsConfig": {
            "description": "L2 smart contract addresses",
            "type": "object",
            "properties": {
                "BridgeAddr": {
                    "description": "BridgeAddr is the address of the bridge smart contract on L2",
                    "type": "string",
                    "example": "0xabcdef1234567890abcdef1234567890abcdef12"
                },
                "GlobalExitRootAddr": {
                    "description": "GlobalExitRootAddr is the address of the GER smart contract on L2",
                    "type": "string",
                    "example": "0xabcdef1234567890abcdef1234567890abcdef12"
                }
            }
        },
        "types.L2GERSyncComponentConfig": {
            "description": "Public configuration of the L2GERSync component, plus its detected sync mode",
            "type": "object",
            "properties": {
                "block_finality": {
                    "description": "BlockFinality is the block finality used by this syncer when querying the chain",
                    "type": "string",
                    "example": "FinalizedBlock"
                },
                "initial_block": {
                    "description": "InitialBlock is the first block that was queried when starting the sync from scratch",
                    "type": "integer",
                    "example": 12345
                },
                "sync_block_chunk_size": {
                    "description": "SyncBlockChunkSize is the amount of blocks queried per request",
                    "type": "integer",
                    "example": 100
                },
                "sync_mode": {
                    "description": "SyncMode is the GER manager mode detected on L2 at startup: \"Legacy\" or \"SovereignChain\".\nNot configuration — auto-detected by probing the L2 GER contract — but useful operational\ninformation. Empty when this instance isn't running the L2GERSync component.",
                    "type": "string",
                    "example": "SovereignChain"
                }
            }
        },
        "types.L2GERSyncInfo": {
        ...
        "types.PublicComponentsConfig": {
            ...
            "properties": {
                ...
                "L2GERSync": {
                    "$ref": "#/definitions/types.L2GERSyncComponentConfig"
                }
            }
        },
        "types.PublicConfigResponse": {
            "description": "Public, non-sensitive configuration of the bridge service",
            "type": "object",
            "properties": {
                "components": {
                    "description": "Components holds the public parameters of each syncer component",
                    "allOf": [
                        {
                            "$ref": "#/definitions/types.PublicComponentsConfig"
                        }
                    ]
                },
                "contracts": {
        ...
        "types.SettlementResponse": {
            "description": "A single L1 settlement event for this network's local exit root",
            "type": "object",
            "properties": {
                "block_hash": {
                    "description": "L1 block's hash, when available (nil if the block table has no recorded hash for it).",
                    "type": "string",
                    "example": "0xabc1234567890abcdef1234567890abcdef1234567890abcdef1234567890a"
                },
                "block_number": {
                    "description": "L1 block this settlement was mined in.",
                    "type": "integer",
                    "example": 123456
                },
                "block_timestamp": {
                    "description": "L1 block's timestamp, in seconds since the Unix epoch. Omitted for the same reason as\nTxHash.",
                    "type": "integer",
                    "example": 1684500000
                },
                "new_local_exit_root": {
                    "description": "Local exit root this settlement produced.",
                    "type": "string",
                    "example": "0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef12345678"
                },
                "tx_hash": {
                    "description": "L1 transaction that settled this certificate. Omitted for settlements synced before this\nfield was tracked (#1817); it is never backfilled for those.",
                    "type": "string",
                    "example": "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcd"
                }
            }
        },
        "types.SettlementsResult": {
            "description": "Paginated response of L1 certificate settlements for this network",
            "type": "object",
            "properties": {
                "count": {
                    "description": "Total number of settlements for this network",
                    "type": "integer",
                    "example": 10
                },
                "settlements": {
                    "description": "List of settlements, most recent first",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/types.SettlementResponse"
                    }
                }
            }
        },
        "types.SyncComponentConfig": {
