...
func newBridgeSync(
	...
	if lastProcessedBlock < cfg.InitialBlockNum {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to process initial block %d: %w", cfg.InitialBlockNum, err)
		}
		logger.Infof("successfully initialized from block %d", cfg.InitialBlockNum)
	}
