...
func TestSubscribeToSync(t *testing.T) {
	ctx := t.Context()
	bridgeSyncCfg := Config{
		DBPath:                             path.Join(t.TempDir(), "TestGetLastRoot.sqlite"),
		...
		InitialBlockNum:                    0,
		WaitForNewBlocksPeriod:             cfgtypes.NewDuration(time.Second * 10),
		RetryAfterErrorPeriod:              cfgtypes.NewDuration(time.Second * 5),
		MaxRetryAttemptsAfterError:         3,
		RequireStorageContentCompatibility: false,
		DBQueryTimeout:                     cfgtypes.NewDuration(30 * time.Second),
	}
	...
	mockReorgDetector := mocksbridgesync.NewReorgDetector(t)
	mockReorgDetector.EXPECT().Subscribe(mock.Anything).Return(nil, nil)
	mockReorgDetector.EXPECT().GetFinalizedBlockType().Return(aggkittypes.LatestBlock)
	mockReorgDetector.EXPECT().String().Return("mockReorgDetector")

	numOfBlocksToProcess := 10
	blocks := make(map[uint64]sync.Block, numOfBlocksToProcess)
	for i := range numOfBlocksToProcess {
		blocks[uint64(i)] = sync.Block{
			Num:  uint64(i),
	...
	subscription := s.SubscribeToSync("test-subscriber", numOfBlocksToProcess)
	require.NotNil(t, subscription)

	// Simulate a new block being processed
	go func() {
		for i := range numOfBlocksToProcess {
	...
	for syncNotification := range subscription.BlockCh {
		b, exists := blocks[syncNotification.Block.Num]
		require.True(t, exists)
		require.Equal(t, syncNotification.Block.Hash, b.Hash)
	}
}
