...
var (
	...
	backwardLETEventSignature = crypto.Keccak256Hash([]byte(
		"BackwardLET(uint256,bytes32,uint256,bytes32)",
	))
...
func buildAppender(
	...
	case SovereignChain:
		...
		appender[backwardLETEventSignature] = buildBackwardLETHandler(bridgeDeployment.agglayerBridgeL2)
	}
...
// buildBackwardLETHandler creates a handler for the BackwardLET event log
func buildBackwardLETHandler(contract *agglayerbridgel2.Agglayerbridgel2) func(*sync.EVMBlock, types.Log) error {
	return func(b *sync.EVMBlock, l types.Log) error {
