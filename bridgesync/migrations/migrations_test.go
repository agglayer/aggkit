...
func TestMigration0003(t *testing.T) {
	...
	_, err = tx.Exec(`
		...
		INSERT INTO token_mapping (
			...
			metadata,
			calldata
		) VALUES (1, 0, 1739270804, '0xabcd', 2, '0x3', '0x5', NULL, 'calldata');
		...
		INSERT INTO bridge (
			...
			tx_hash,
			from_address
		) VALUES (1, 0, 0, 0, '0x3', 0, '0x0000', 0, NULL, 'calldata', 0, 1739270804, '0xabcd', '0x123');
	...
	require.Equal(t, common.HexToAddress("0x3"), tokenMapping.OriginTokenAddress)
	require.Equal(t, common.HexToAddress("0x5"), tokenMapping.WrappedTokenAddress)
	require.Equal(t, []byte("calldata"), tokenMapping.Calldata)
	...
	require.NotNil(t, bridge)
	require.Equal(t, uint64(1739270804), bridge.BlockTimestamp)
	require.Equal(t, []byte("calldata"), bridge.Calldata)
