...
func TestGetClaimByGlobalIndex(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "successful retrieval of latest claim",
			setupClaims: []*Claim{
				{
					BlockNum:            5,
					BlockPos:            1,
					GlobalIndex:         GenerateGlobalIndex(true, 0, 100),
					OriginNetwork:       1,
					...
					ProofLocalExitRoot:  types.Proof{},
					ProofRollupExitRoot: types.Proof{},
					MainnetExitRoot:     common.HexToHash("0x3333333333333333333333333333333333333333333333333333333333333333"),
			...
			expectedError: "",
			description:   "should return the claim when it exists before the given block number",
		},
		{
			name: "returns latest claim when multiple claims exist with same global index",
			setupClaims: []*Claim{
				{
					BlockNum:            3,
					BlockPos:            0,
					GlobalIndex:         GenerateGlobalIndex(false, 5, 200),
					OriginNetwork:       1,
					OriginAddress:       common.HexToAddress("0x1111111111111111111111111111111111111111"),
					DestinationAddress:  common.HexToAddress("0x2222222222222222222222222222222222222222"),
			...
			expectedError: "",
			description:   "should return the latest claim (highest block_num and block_pos) when multiple claims exist",
		},
		{
			name: "claim not found when global index doesn't exist",
			setupClaims: []*Claim{
				{
					BlockNum:            5,
					BlockPos:            0,
			...
			globalIndex:   GenerateGlobalIndex(false, 10, 500),
			blockNumber:   10,
			expectedClaim: nil,
			expectedError: "claim not found with globalIndex:",
			description:   "should return error when global index doesn't exist",
		},
		{
			name: "claim not found when all claims are at or after the given block number",
			setupClaims: []*Claim{
				{
					BlockNum:            10,
					BlockPos:            0,
			...
			expectedClaim: nil,
			expectedError: "claim not found with globalIndex:",
			description:   "should return error when all claims are at or after the given block number",
		},
		{
			...
			expectedClaim: nil,
			expectedError: "claim not found with globalIndex:",
			description:   "should return error when database is empty",
		},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Setup test database
			dbPath := path.Join(t.TempDir(), fmt.Sprintf("bridgesyncTestGetClaimByGlobalIndex_%s.sqlite", tc.name))
			err := migrations.RunMigrations(dbPath)
			require.NoError(t, err)
			...
			ctx := context.Background()
			result, err := p.GetClaimByGlobalIndex(ctx, tc.globalIndex, tc.blockNumber)
			...
			if tc.expectedError != "" {
				...
				require.NoError(t, err)
				require.NotNil(t, tc.expectedClaim)
				require.Equal(t, *tc.expectedClaim, result)
			}
		})
	}
}
