...
func TestRateLimit(t *testing.T) {
	...
	sut := common.NewRateLimit(common.NewRateLimitConfig(2, 2*time.Second))
	// This call is register because no sleep time // req:[0sec]
	require.Nil(t, sut.Call("test", common.NoSleepReturnTime))
	common.TimeProvider = func() time.Time {
		return now.Add(time.Second * 1)
	}
	// This call is register because no sleep time // req:[0sec, 1 sec]
	require.Nil(t, sut.Call("test", common.NoSleepReturnTime))
	// This call is not register because it hits 2 calls and have to wait 1 second
	sleepTime := sut.Call("test", common.NoSleepReturnTime)
	require.NotNil(t, sleepTime)
	require.Equal(t, time.Second, *sleepTime)
	// The previous call is not registered so it will return the same
	sleepTime2 := sut.Call("test", common.NoSleepReturnTime)
	require.Equal(t, sleepTime, sleepTime2, "must return the same as previous call")
	...
	// These calls should succeed again 1 call 0 sec // req:[1sec, 2sec] (delete 0sec)
	require.Nil(t, sut.Call("test", common.NoSleepReturnTime))
	// This must sleep another second ()
	sleepTime = sut.Call("test", common.NoSleepReturnTime)
	require.Equal(t, time.Second, *sleepTime)
}
...
func TestRateLimitSleepTime(t *testing.T) {
	...
	start := time.Now()
	sleepTime = sut.Call("test", common.AllowToSleep)
