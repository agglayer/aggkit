...
The Aggsender Validator serves as an independent validation layer that verifies the correctness of certificates generated by the AggSender Proposer. It acts as a quality gate, ensuring that only properly constructed certificates are submitted to the AggLayer, preventing invalid submissions that could cause issues in the cross-chain protocol. It is also a part of a security layer, acting as a member of a validators committee whose signature acts as a vote for corecctness of the new certificate. When a threshold of signatures for a new certificate is reached, a certificate can be accepted by the Agglayer.
...
- **Metadata Compatibility**: Ensures certificate metadata is compatible with current version
...
- **Certificate Reconstruction**: Rebuilds the certificate using the same flow and compares with incoming certificate
...
│  │                                                             │    │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │    │
│  │  │Certificate  │  │ Signature   │  │   Threshold         │  │    │
│  │  │ Builder     │  │ Collector   │  │   Checker           │  │    │
│  │  └─────────────┘  └─────────────┘  └─────────────────────┘  │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                               │                                     │
│                               ▼                                     │
...
The validator is configured using the following parameters:

```toml
[Validator]
# Enable RPC endpoint for the validator (only used for local testing in development)
EnableRPC = false

# gRPC server configuration of the aggsender-validator grpc service
...
# his allows clients to introspect the server's services and methods
EnableReflection = false
# MaxDecodingMessageSize is the maximum size of a message that the gRPC server can receive
MaxDecodingMessageSize = 26214400  # 25Mb

# Signer configuration for certificate signing
[Validator.Signer]
Method = "local"  # or "remote"
Path = "/path/to/keystore"
Password = "password"

# Maximum certificate size (0 = unlimited)
MaxCertSize = 1048576  # 1MB

# Maximum L2 block number for certificates (0 = disabled)
# This option is only used in upgrades of networks which require all previous
# bridge transactions to be settled in one large certificate, so this value
# tells the aggsender and aggsender validator until which block a certificate
# needs to be built
MaxL2BlockNumber = 0

# LocalExitRoot (LER) Querier configuration
[Validator.LerQuerierConfig]
# Address of the RollupManager contract on L1
RollupManagerAddr = "0x1234..."
# Block on which given networks rollup was created on L1
RollupCreationBlockL1 = 1000000

# Pessimistic Proof specific configuration
[Validator.PPConfig]
# This tells the validator that at least one bridge exit needs to exist in
# certificates in Pessimistic networks (this needs to be configured the same as in the aggsender proposer)
RequireOneBridgeInPPCertificate = true

# AggLayer client configuration
[Validator.AgglayerClient]
# Tells the validator should it cache the results gotten from agglayer
Cached = true
    [Validator.AgglayerClient.ConfigurationCache]
        # Amount of time a cached request result from agglayer will be stored
		TTL = "5m"
        # Number of results that can be stored in cache
		Capacity = 100
    [Validator.AgglayerClient.GRPC]
        # URL of the agglayer service
        URL = "localhost:9090"
        # Min connection timeout for the agglayer service connection
        MinConnectTimeout = "5s"
        # Request timeout for outgoing requests to agglayer service
        RequestTimeout = "300s"
        # Is connection to agglayer service configured as TLS
        UseTLS = false
        # Retry configuration for outgoing requests to agglayer service
