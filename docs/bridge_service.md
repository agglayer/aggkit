...
## Bridge Service Finder

The bridge service finder is an optional, lightweight component that resolves and serves the bridge
service REST URL for every network (rollup) attached to the L1 rollup manager. Consumers (e.g. other
aggkit instances or tooling) can query it to discover which URL currently serves the bridge service
for a given `networkID`, instead of hardcoding it.

It builds an in-memory `networkID -> URL` cache by enumerating the rollups attached to
`RollupManagerAddr`, resolving each network's URL (in priority order: a static config override, the
aggchain contract's `aggchainMetadata["BRIDGE_SERVICE_URL"]`, or the rollup's
`trustedSequencerURL()` with the bridge REST port substituted in), probing it via `/health`, and then
keeping the cache fresh with a finality-bounded on-chain event polling loop.
...
# Optional static networkID -> bridge service URL overrides. Also the only way to set a URL for
# network 0 (L1), which is not enumerated on-chain. Entries here are never overwritten by on-chain events.
# URLs = { 0 = "http://localhost:5577" }
```

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `Enabled` | `bool` | `false` | Whether the bridge service finder is started. Disabled by default since it requires a meaningful `RollupManagerAddr` to be useful. |
| `RollupManagerAddr` | `address` | `{{L1NetworkConfig.RollupManagerAddr}}` | Address of the RollupManager / AgglayerManager contract on L1 from which attached networks (rollups `1..RollupCount`) are enumerated. Reuses the same value as `L1NetworkConfig.RollupManagerAddr` by default. |
| `URLs` | `map[uint32]string` | `{}` (empty) | Highest-priority static override map from `networkID` to bridge service URL. Entries here are terminal and never overwritten by on-chain events. The only way to configure a URL for network 0 (L1). |
| `BlockFinality` | `string` | `"FinalizedBlock"` | Finality level (`PendingBlock`, `LatestBlock`, `SafeBlock`, `FinalizedBlock`, `EarliestBlock`) used to bound the upper block of each event scan, so the finder does not react to logs that may be reorged away. |
| `PollInterval` | `duration` | `"30s"` | Period between event-scan iterations of the listener loop. |
| `BlockChunkSize` | `uint64` | `10000` | Maximum number of blocks queried per `FilterLogs` request while scanning. |
| `HealthCheckPath` | `string` | `"/health"` | HTTP path appended to a resolved bridge service URL to probe liveness. |
| `HealthCheckTimeout` | `duration` | `"5s"` | Timeout applied to each health-check HTTP request. |
| `RequireAllHealthyOnStart` | `bool` | `false` | If `true`, `Start` fails when any resolved bridge service is unreachable during initial cache construction. If `false` (default), unreachable services are cached as unhealthy and may be healed by a later on-chain update. |

The bridge service finder shares the same L1 RPC client as the rest of aggkit
(`L1NetworkConfig.RPC`); no separate RPC endpoint needs to be configured for it.
