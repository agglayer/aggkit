...
| WaitingL1InfoLeafAvailable | `ger` (Hash), `block_number` (uint64), `block_timestamp` (uint64) | Always right before `WaitingClaim`, on every route (see [#1823](https://github.com/agglayer/aggkit/issues/1823)): the covering L1 info tree leaf, resolved from whichever earlier step produced it (`WaitL1SettledGER`'s settlement for an L2-originated bridge, `WaitingGERUpdate` for an L1-originated one), as seen by the destination network's own bridge-service instance — `block_number`/`block_timestamp` are the L1 event that produced the leaf, the same fields as `InjectedGERResult`'s own `l1_info_tree_leaf` (see [InjectedGERResult](#injectedgerresult)), just not nested under it since there is no destination-side injection to report alongside |
