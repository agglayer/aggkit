...
func TestE2E(t *testing.T) {
	...
	syncer, err := l1infotreesync.New(ctx, cfg, aggkittypes.LatestBlock, rdm, client.Client(),
...
func TestWithReorgs(t *testing.T) {
	...
	syncer, err := l1infotreesync.New(ctx, cfg, aggkittypes.LatestBlock, rd, client.Client(),
		l1infotreesync.FlagAllowWrongContractsAddrs, aggkittypes.SafeBlock)
	require.NoError(t, err)
	go syncer.Start(ctx)
