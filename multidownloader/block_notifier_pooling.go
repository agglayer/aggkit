...
// BlockNotifierPolling implements BlockNotifier that uses polling to notify new blocks
type BlockNotifierPolling struct {
	...
	aggkitcommon.PubSub[mdrtypes.EventNewBlock]
}

// NewBlockNotifierPolling creates a new BlockNotifierPolling.
// if param `subscriber` is nil a new GenericSubscriberImpl[mdrtypes.EventNewBlock] will be created.
...
func NewBlockNotifierPolling(ethClient aggkittypes.BaseEthereumClienter,
	config ConfigBlockNotifierPolling,
	logger aggkitcommon.Logger,
	subscriber aggkitcommon.PubSub[mdrtypes.EventNewBlock]) (*BlockNotifierPolling, error) {
	if subscriber == nil {
		subscriber = aggkitcommon.NewGenericSubscriber[mdrtypes.EventNewBlock]()
	}
...
func (b *BlockNotifierPolling) step(ctx context.Context,
	...
	*blockNotifierPollingInternalStatus, *mdrtypes.EventNewBlock) {
	...
	// New blockNumber!
	eventToEmit := &mdrtypes.EventNewBlock{
