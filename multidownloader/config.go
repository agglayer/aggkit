...
var (
	...
	ErrInvalidMaxParallelBlockHeaderRetrieval = errors.New("MultidownloaderConfig.MaxParallelBlockHeaderRetrieval: max parallel block header retrieval must be greater than 0")
	// ErrInvalidWaitPeriodToCheckCatchUp is returned when the wait period to check catch up is invalid
	ErrInvalidWaitPeriodToCheckCatchUp = errors.New("MultidownloaderConfig.WaitPeriodToCheckCatchUp: wait period to check catch up must be greater than 0")
)
...
func NewConfigDefault(name string) Config {
	return Config{
		...
		WaitPeriodToCheckCatchUp:        types.NewDuration(time.Second * 10),
	}
}
