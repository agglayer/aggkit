...
@test "L1 to L2" {
    if [ -z $bridge_api_url ]; then
        echo "error bridge_api: $bridge_api_url"
        exit 1
    fi
    echo "=== Running LxLy bridge eth L2 to L1 amount:$amount" >&3
    destination_net=0
    bridge_asset "$native_token_addr" "$l2_rpc_url"


    timeout="200" 
    claim_frequency="60"
    echo "=== Running LxLy claim L1($l1_rpc_url) bridge_api: $bridge_api_url for: $bridge_tx_hash" >&3
    claim_tx_hash "$timeout" "$bridge_tx_hash" "$destination_addr" "$l1_rpc_url"  "$bridge_api_url"
