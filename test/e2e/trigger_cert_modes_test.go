...
func TestTriggerCertModes(t *testing.T) {
	...
	// Accept both EpochBased (when TriggerCertMode is absent/Auto, the default for PessimisticProof)
	// and ASAP (when explicitly configured, as generated by recent kurtosis-cdk snapshots).
	require.True(t, effectiveTrigger == triggerCertModeEpochBased || effectiveTrigger == triggerCertModeASAP,
		"op-pp PessimisticProof TriggerCertMode must be EpochBased (Auto) or ASAP (explicit); got %q", effectiveTrigger)
