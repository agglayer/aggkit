...
`options.l1EndBlock` (optional, `uint64`) is the L1 cutoff block for the L1 reads. When set (> 0), Step E scans L1 for unclaimed deposits only up to this block (filtering the bridge service cross-check to match) and Step I starts its backward `UpdateL1InfoTreeV2` scan from it, so post-snapshot L1 deposits cannot block the pipeline (AET-03). Pick a block at or after the moment the sequencer was stopped. `0` (the default) means no cutoff: the latest L1 block is used (previous behaviour). A value below `options.l1StartBlock` is rejected by `LoadConfig`.
