...
- **Does:** first resolves `targetBlock` (finality keyword, optional offset, or concrete number) to a `uint64` via an RPC call when needed; right after the resolution — before the LBT scan — re-runs the AET-11 unsettled-bridge-exits verification on the resolved block (`assertNoUnsettledBridgeExits`, hard abort on mismatch — a warning instead with `options.ignoreLERMismatch=true`; skipped with a warning when `agglayerClient.grpc.url` is unset, which Step CHECK reports as a failure); then scans L2 bridge `NewWrappedToken` events, fetches `totalSupply` per token at the resolved block, computes unlocked native balance.
...
- One per token with SC-locked value > 0 → destination is `exitAddress` on `destinationNetwork`.
  **Omitted entirely with `options.skipSCLockedValue=true`**: those funds are intentionally left
  behind (Step C still runs and its output records them; Step F discounts them from the
  LBT/agglayer balances). Holder-bridge exits (Step C `HolderBridges`) are unaffected.
...
- **Skipped SC-locked discount:** with `options.skipSCLockedValue=true` the certificate omits the SC-locked exits (Step D), so Step F discounts each token's `PendingSCLockedBalance` (Step C) from the **LBT and agglayer amounts** via `discountSkippedSCLocked` before comparing — the check remains strict equality and the cap budget (`RemainingBalance`) excludes the left-behind funds. The applied discount is recorded per token as `skippedSCLockedAmount` in `step-f-checks.json` (the persisted LBT/agglayer amounts are the adjusted ones). In single-step mode `--step f` then requires `step-c-sc-locked-values.json` (run Step C first).
...
- **Fetches `L1InfoTreeLeafCount`** — scans L1 backwards from `options.l1EndBlock`
...
Required: `l2RpcUrl`, `l2BridgeAddress`, `exitAddress` (unless `options.skipSCLockedValue=true`),
`targetBlock`.
...
only be recovered by signing from it). With `options.skipSCLockedValue=true` the SC-locked funds are
left behind and `exitAddress` is unused: the presence and zero-address checks are skipped (a
non-empty value must still be valid hex).
...
`options.l1EndBlock` (optional, `uint64`) is the L1 cutoff block for the L1 reads. When set (> 0), Step E scans L1 for unclaimed deposits only up to this block (filtering the bridge service cross-check to match) and Step I starts its backward `UpdateL1InfoTreeV2` scan from it, so post-snapshot L1 deposits cannot block the pipeline (AET-03). Pick a block at or after the moment the sequencer was stopped. `0` (the default) means no cutoff: the latest L1 block is used (previous behaviour). A value below `options.l1StartBlock` is rejected by `LoadConfig`; a value beyond the current L1 head is rejected at run time by `resolveL1EndBlock` (shared by Steps E and I) — it is almost surely a misconfiguration (e.g. an L2 block number), and some L1 clients reject `eth_getLogs` ranges past the head with a cryptic "invalid block range params" error.
...
- `options.skipSCLockedValue` — `false` (default). When `true`, Step D omits the per-token SC-locked exits (`PendingSCLockedBalance` → `exitAddress`) from the certificate: those funds are intentionally left behind on the L2, and `exitAddress` is no longer required. Step C still runs — `step-c-sc-locked-values.json` records what is left behind — and Step F discounts the omitted amounts from the LBT/agglayer balances via `discountSkippedSCLocked` (the check stays strict equality; each token's `skippedSCLockedAmount` is recorded in `step-f-checks.json`, and the cap budget excludes the omitted funds). Holder-bridge exits are unaffected. In single-step mode, `--step f` then requires `step-c-sc-locked-values.json`.
...
- `options.ignoreLERMismatch` — `false` (default). When `true`, the AET-11 unsettled-bridge-exits verification (Step CHECK check 9 via `reportUnsettledExitsFailure`, the Step 0 guard and Step H's LER cross-check via `suppressUnsettledExitsError`) is downgraded from an abort to a warning; the Step CHECK result still records the real outcome. The certificate then chains from a `PrevLocalExitRoot` that does not cover every emitted bridge exit, so the agglayer will most likely reject it — enable only to inspect such a snapshot knowingly. Step H's pending-certificate guard is unaffected.
