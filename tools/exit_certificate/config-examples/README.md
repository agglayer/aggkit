...
| `exitAddress` | The address that will receive assets locked in smart contracts. **Required** — and it **must not be the zero address** (`0x00…00`); the tool errors otherwise. **You must hold the private key for this address** — funds can only be recovered by signing from it after the certificate settles. |
