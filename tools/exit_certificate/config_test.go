...
func TestLoadConfig_IgnoreAddresses(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, []common.Address{
		...
		common.HexToAddress("0x0000000000000000000000000000000000000def"),
	}, cfg.Options.IgnoreAddresses)

	set := cfg.Options.ignoreAddressSet()
	require.Len(t, set, 2)
	_, ok := set[common.HexToAddress("0x0000000000000000000000000000000000000abc")]
	require.True(t, ok)
}

func TestLoadConfig_InvalidIgnoreAddress(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "bad-ignore.json")
	data := `{
		"l2RpcUrl": "http://localhost:8545",
		"l2BridgeAddress": "0x2a3DD3EB832aF982ec71669E178424b10Dca2EDe",
		"exitAddress": "0x0000000000000000000000000000000000000001",
		...
		"options": {
			"useAgglayerAdminToStepFCheck": false,
			"ignoreAddresses": ["not-an-address"]
		}
	}`
	require.NoError(t, os.WriteFile(path, []byte(data), 0o600))
	...
	_, err := LoadConfig(path)
	require.Error(t, err)
	require.Contains(t, err.Error(), "ignoreAddresses")
	require.Contains(t, err.Error(), "not a valid hex address")
}

func TestLoadConfig_ZeroIgnoreAddress(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "zero-ignore.json")
	data := `{
		"l2RpcUrl": "http://localhost:8545",
		"l2BridgeAddress": "0x2a3DD3EB832aF982ec71669E178424b10Dca2EDe",
		"exitAddress": "0x0000000000000000000000000000000000000001",
		...
		"options": {
			"useAgglayerAdminToStepFCheck": false,
			"ignoreAddresses": ["0x0000000000000000000000000000000000000000"]
		}
	}`
	require.NoError(t, os.WriteFile(path, []byte(data), 0o600))
