...
Connection info comes from the environment (see export_kurtosis_env.sh / export_e2e_env.sh).
...
check_env() {
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        ...
        log_error "                or: source <(tools/exit_certificate/scripts/export_e2e_env.sh $NETWORK_INDEX)"
        exit 1
    fi
}
