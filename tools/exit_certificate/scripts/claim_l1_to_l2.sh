#!/usr/bin/env bash
# Claims a pending L1 -> L2 bridge deposit by calling claimAsset/claimMessage on the
# L2 bridge contract, using the Merkle proof served by the destination network's
# aggkit bridge service (the same data source autoclaim uses to claim automatically).
# Useful when auto-claim is disabled/not running and a deposit made with
# bridge_l1_to_l2.sh never shows up as claimed on L2.
#
...
# BRIDGE_SERVICE_URL is not covered by those helpers; set it to the destination
# network's aggkit bridge service REST API (e.g. http://localhost:11577).
#
# Requires: cast (Foundry), python3.
#
# Required environment variables:
#   L2_RPC_URL          L2 JSON-RPC URL (where claimAsset/claimMessage is sent)
#   BRIDGE_ADDR         Bridge contract address (same address on L1 and L2)
#   BRIDGE_SERVICE_URL  aggkit bridge service REST API for the destination L2 network
#   L1_RPC_URL          L1 JSON-RPC URL (only required with --tx-hash)
set -euo pipefail
...
usage() {
    cat >&2 <<EOF
Usage: $0 (-c DEPOSIT_COUNT | -x TX_HASH) [OPTIONS]

Claims a pending L1 -> L2 bridge deposit on the L2 bridge contract, fetching
the Merkle proof from the destination network's aggkit bridge service.

Options:
  -c, --deposit-count N        L1 deposit count to claim (mutually exclusive with --tx-hash)
  -x, --tx-hash       HASH     L1 bridge tx hash to extract the deposit count from
                                (requires L1_RPC_URL; typically the hash printed by
                                bridge_l1_to_l2.sh)
...
Required environment variables:
  L2_RPC_URL                  L2 JSON-RPC URL (where the claim tx is sent)
...
Examples:
  ...
  $0 --tx-hash 0xabc...           # claim the deposit made by that L1 tx
  $0 --deposit-count 2            # claim L1 deposit_count=2 directly
EOF
    exit 1
}
...
# L1 is always source/origin network 0 — this script only handles L1 -> L2 claims.
SOURCE_NETWORK=0
...
check_env() {
    ...
    [[ -n "$TX_HASH" && -z "$L1_RPC_URL" ]] && missing+=("L1_RPC_URL (required with --tx-hash)")
    if [[ ${#missing[@]} -gt 0 ]]; then
        log_error "Missing required environment variables: ${missing[*]}"
        log_error "Populate them with: source <(tools/exit_certificate/scripts/export_kurtosis_env.sh 1)"
        log_error "                or: source <(tools/exit_certificate/scripts/export_e2e_env.sh 1)"
        log_error "BRIDGE_SERVICE_URL is not covered by those helpers — set it to the destination"
...
if [[ -n "$TX_HASH" ]]; then
    log_info "Extracting deposit count from L1 tx $TX_HASH..."
    DEPOSIT_COUNT=$(extract_deposit_count "$TX_HASH" "$L1_RPC_URL")
fi

log_info "Source network:      L1 (network $SOURCE_NETWORK)"
...
# ---------------------------------------------------------------------------
# Fetch the bridge details from the destination bridge service
# ---------------------------------------------------------------------------

log_info "Fetching bridge details..."
BRIDGE_HTTP_CODE=$(curl -s -o /tmp/claim_l1_to_l2_bridge.json -w "%{http_code}" \
    "${BRIDGE_SERVICE_URL}/bridge/v1/bridge-by-deposit-count?network_id=${SOURCE_NETWORK}&deposit_count=${DEPOSIT_COUNT}")
BRIDGE_JSON=$(cat /tmp/claim_l1_to_l2_bridge.json)
rm -f /tmp/claim_l1_to_l2_bridge.json
...
IS_CLAIMED_RESULT=$(cast call --rpc-url "$L2_RPC_URL" "$BRIDGE_ADDR" "$IS_CLAIMED_CALLDATA" 2>/dev/null || echo "0x0")
IS_CLAIMED_VAL=$(cast --to-dec "${IS_CLAIMED_RESULT:-0x0}" 2>/dev/null || echo "0")
if [[ "$IS_CLAIMED_VAL" != "0" ]]; then
    log_info "Deposit count=$DEPOSIT_COUNT is already claimed on L2 — nothing to do."
    exit 0
fi

# ---------------------------------------------------------------------------
# Resolve the L1 info tree index covering this deposit, then fetch its proof
# ---------------------------------------------------------------------------

log_info "Resolving covering L1 info tree index..."
L1_INDEX_HTTP_CODE=$(curl -s -o /tmp/claim_l1_to_l2_index.json -w "%{http_code}" \
    "${BRIDGE_SERVICE_URL}/bridge/v1/l1-info-tree-index?network_id=${SOURCE_NETWORK}&deposit_count=${DEPOSIT_COUNT}")
L1_INDEX_BODY=$(cat /tmp/claim_l1_to_l2_index.json)
rm -f /tmp/claim_l1_to_l2_index.json
...
if [[ "$L1_INDEX_HTTP_CODE" != "200" ]]; then
    ...
    if [[ "$L1_INDEX_BODY" == *"has not been included"* ]]; then
        log_error "The Global Exit Root has not been updated to cover this deposit yet."
        log_error "Wait for the next GER injection on network $DESTINATION_NETWORK and retry."
    fi
    exit 1
fi
...
PROOF_HTTP_CODE=$(curl -s -o /tmp/claim_l1_to_l2_proof.json -w "%{http_code}" \
    "${BRIDGE_SERVICE_URL}/bridge/v1/claim-proof?network_id=${SOURCE_NETWORK}&leaf_index=${LEAF_INDEX}&deposit_count=${DEPOSIT_COUNT}")
PROOF_JSON=$(cat /tmp/claim_l1_to_l2_proof.json)
rm -f /tmp/claim_l1_to_l2_proof.json
...
log_info "Calling $METHOD on L2 bridge..."
...
TX_STATUS=$(cast receipt --rpc-url "$L2_RPC_URL" "$TX_HASH_OUT" status 2>/dev/null || true)
TX_BLOCK=$(cast receipt --rpc-url "$L2_RPC_URL" "$TX_HASH_OUT" blockNumber 2>/dev/null || true)
if [[ -z "$TX_STATUS" ]]; then
    ...
    log_info "Deposit count=$DEPOSIT_COUNT claimed successfully on L2 network $DESTINATION_NETWORK."
else
    log_error "Receipt: status=REVERTED blockNumber=$TX_BLOCK"
    log_error "Replaying transaction to get revert reason..."
    cast run --rpc-url "$L2_RPC_URL" "$TX_HASH_OUT" 2>&1 | grep -E "revert|Revert|error|Error|←" | head -20 >&2
    exit 1
fi
