...
func TestGetRootByIndexAndBlockNum(t *testing.T) {
	t.Parallel()

	ctx := context.Background()

	t.Run("Valid root retrieval", func(t *testing.T) {
		t.Parallel()

		treeDB := createTreeDBForTest(t)
		merkleTree := NewAppendOnlyTree(treeDB, "")

		// Insert a root into the database
		tx, err := db.NewTx(ctx, treeDB)
		require.NoError(t, err)
		...
		require.NoError(t, merkleTree.storeRoot(tx, expectedRoot))
		require.NoError(t, tx.Commit())

		// Retrieve the root
		root, err := merkleTree.GetRootByIndexAndBlockNum(ctx, 1, 100)
		require.NoError(t, err)
		require.NotNil(t, root)
		require.Equal(t, expectedRoot.Hash, root.Hash)
		require.Equal(t, expectedRoot.BlockPosition, root.BlockPosition)
		require.Equal(t, expectedRoot.BlockNum, root.BlockNum)
	})
	...
	t.Run("Root not found", func(t *testing.T) {
		...
		// Attempt to retrieve a non-existent root
		root, err := merkleTree.GetRootByIndexAndBlockNum(context.Background(), 1, 100)
		require.ErrorIs(t, err, db.ErrNotFound)
		require.Nil(t, root)
	})

	t.Run("Root retrieval with blockNum constraint", func(t *testing.T) {
		t.Parallel()

		treeDB := createTreeDBForTest(t)
		merkleTree := NewAppendOnlyTree(treeDB, "")

		// Insert multiple roots into the database
		tx, err := db.NewTx(context.Background(), treeDB)
		require.NoError(t, err)
		root1 := types.Root{
			BlockPosition: 0,
		...
		require.NoError(t, merkleTree.storeRoot(tx, root1))
		require.NoError(t, merkleTree.storeRoot(tx, root2))
		require.NoError(t, tx.Commit())

		// Retrieve the root with blockNum constraint
		root, err := merkleTree.GetRootByIndexAndBlockNum(context.Background(), 1, 75)
		require.NoError(t, err)
		require.NotNil(t, root)
		require.Equal(t, root1.Hash, root.Hash)
		require.Equal(t, root1.BlockNum, root.BlockNum)
	})
}
